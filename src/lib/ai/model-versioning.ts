 ...
 * Phase 6: Downgrade model when budget threshold exceeded (getEffectiveModel)
 */
import { db } from "@/lib/firebase-admin";
import { getEffectiveModel } from "@/lib/ai/cost-governance";
...
export async function getModelConfig(orgId?: string | null): Promise<ModelConfig> {
  ...
  if (orgId) {
    const effectiveModel = await getEffectiveModel(orgId);
    if (effectiveModel && effectiveModel !== base.model_name) {
      return { ...base, model_name: effectiveModel };
    }
  }
  return base;
}

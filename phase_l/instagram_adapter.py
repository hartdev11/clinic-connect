...
class InstagramAdapter:
    ...
    def send(self, response_package: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if response_package.get("source_platform") != "instagram": errors.append("invalid_platform")
        if errors: return {"status":"error","platform":"instagram","errors":errors}
        ...
        return {"status":"ok","platform":"instagram","delivery_status":"mock_sent" if self.mock_mode else "sent","external_user_id":response_package["external_user_id"],"outbound_payload":payload}
